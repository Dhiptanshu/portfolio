...
export function SectionManager({ initialItems }: { initialItems: Section[] }) {
  ...
  async function create() {
    ...
    if (response.ok) setItems([...items, await response.json()]);
    setTitle("");
  }
...
export function EntryManager({ sections, initialItems }: { sections: Section[]; initialItems: Entry[] }) {
  ...
  return (
    <CrudList
      ...
      createControl={<><select className="h-10 rounded-md border border-input bg-background px-3 text-sm" value={sectionId} onChange={(e) => setSectionId(e.target.value)}>{sections.map((section) => <option key={section.id} value={section.id}>{section.title}</option>)}</select><Input placeholder="Entry title" value={title} onChange={(e) => setTitle(e.target.value)} /><Button onClick={create} disabled={!title || !sectionId}><Plus className="h-4 w-4" />Create</Button></>}
    />
  );
}
...
function CrudList<T extends Item>({ items, setItems, endpoint, createControl }: { items: T[]; setItems: (items: T[]) => void; endpoint: string; createControl: React.ReactNode }) {
  ...
  async function update(id: string, patch: Partial<T>) {
    const response = await fetch(endpoint, { method: "PATCH", body: JSON.stringify({ id, ...patch }) });
    if (response.ok) setItems(items.map((item) => (item.id === id ? { ...item, ...(patch as T) } : item)));
  }
